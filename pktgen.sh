...
SRC_NET="${SRC}${SRC_PREFIX}"
DST_NET="${DST}${DST_PREFIX}"
...
function config_device {
	...
	ip netns exec at_ns0 ip addr add ${DST_NET} dev veth0
	ip netns exec at_ns0 ip link set dev veth0 up
	ip link set dev veth1 up mtu 1500
	ip addr add dev veth1 ${SRC_NET}
...
function pktgen {
ping -W 2 -w 1 -q -c 1 172.16.1.100
IPN=$(ip neigh get 172.16.1.100 dev veth1)
set ${IPN}
MAC=$5
PKTGEN=../linux/samples/pktgen/pktgen_sample01_simple.sh
PKTGEN3=../linux/samples/pktgen/pktgen_sample01_simple.sh
#${PKTGEN} -i veth1 -d ${DST} -m ${MAC}
${PKTGEN3} -i veth1 -d ${DST} -m ${MAC} -n 1000000 -s 1400
}
